...
from ..config import DELAY, HH_ACCESS_TOKEN, HH_API_URL, MAX_PAGES, USE_MOCK
...
def _map_vacancy(v: dict) -> dict:
    ...
    area = v.get('area', {})
    area_name = (
        area.get('name', 'Неизвестно') if isinstance(area, dict) else str(area)
    )

    skills = [
        {'name': s['name']} for s in v.get('key_skills', []) if s.get('name')
    ][:15]
        ...
        while page < MAX_PAGES:
            try:
                ...
                if resp.status_code == 401:
                    raise RuntimeError(
                        'Токен hh.ru недействителен. Обновите HH_ACCESS_TOKEN в config.py.'
                    )
                ...
                print(
                    f'[HH.ru] Page {page}: got {len(items)} items, total found: {found}'
                )
                ...
                raise RuntimeError(
                    'Не удалось подключиться к API hh.ru. Проверьте интернет.'
                )
